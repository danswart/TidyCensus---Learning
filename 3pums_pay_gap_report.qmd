...
# Add timestamp and progress indicators
...
if (!is.null(physician_results)) {
  ...
  print(kable(physician_formatted, caption = "Physician-Related Occupations (Never-Married Females)"))
} else {
...
for (term in medical_terms) {
  ...
  if (!is.null(results) && nrow(results) > 0) {
    cat("\nFound", nrow(results), "occupations matching '", term, "':\n", sep = "")
    top_result <- results %>% dplyr::slice_head(n = 3)
    for (i in 1:nrow(top_result)) {
      cat("- ", top_result$OCCP_label[i], ": ", dollar(top_result$mean_earnings[i]), 
...
if (!is.null(overall_gap) && !is.null(overall_gap$mean_gap_dollars)) {
  ...
  kable(overall_summary,
        caption = "Overall Pay Gap: Never-Married Females vs All Males (Texas, Age 25-64) - MEAN EARNINGS",
...
# Key Findings (Mean Earnings Analysis)
