...
all_variables <- all_vars_5 |>
  dplyr::filter(stringr::str_detect(concept, "earning|income") &
                stringr::str_detect(concept, "sex") &
                stringr::str_detect(concept, "marital") &
                stringr::str_detect(concept, "occupation") &
                stringr::str_detect(concept, "experience"))

# Result: Zero matches (as expected)
base::nrow(all_variables)
...
never_married_vars <- all_vars_5 |>
  dplyr::filter(stringr::str_detect(base::tolower(concept), "never married") | 
                stringr::str_detect(base::tolower(label), "never married"))
...
base::print(base::paste("Found", base::nrow(never_married_concepts), "different never_married-related concepts"))
...
earnings_vars <- all_vars_5 |>
  dplyr::filter(stringr::str_detect(base::tolower(concept), "earning|income|wage") | 
                stringr::str_detect(base::tolower(label), "earning|income|wage"))
...
base::print(base::paste("Found", base::nrow(earnings_concepts), "different earnings-related concepts"))
...
base::print(base::paste("Found", base::nrow(sex_concepts), "different sex-related concepts"))
...
base::print(base::paste("Found", base::nrow(marital_concepts), "different marital-related concepts"))
...
base::print(base::paste("Found", base::nrow(occupation_concepts), "different occupation-related concepts"))
...
base::print(base::paste("Found", base::nrow(experience_concepts), "different experience-related concepts"))
...
earnings_sex <- all_vars_5 |>
  dplyr::filter((stringr::str_detect(base::tolower(concept), "earning|income|wage")) &
                (stringr::str_detect(base::tolower(concept), "sex|male|female")))

# Earnings × Marital Status  
earnings_marital <- all_vars_5 |>
  dplyr::filter((stringr::str_detect(base::tolower(concept), "earning|income|wage")) &
                (stringr::str_detect(base::tolower(concept), "marital")))

# Earnings × Occupation
earnings_occupation <- all_vars_5 |>
  dplyr::filter((stringr::str_detect(base::tolower(concept), "earning|income|wage")) &
                (stringr::str_detect(base::tolower(concept), "occupation")))

# Display results
combinations_summary <- tibble::tibble(
  combination = base::c("Earnings × Sex", "Earnings × Marital", "Earnings × Occupation"),
  n_variables = base::c(base::nrow(earnings_sex), base::nrow(earnings_marital), base::nrow(earnings_occupation))
)
...
b08_tables <- all_vars_5 |>
  dplyr::filter(stringr::str_starts(name, "B08")) |>
  dplyr::distinct(concept) |>
  dplyr::arrange(concept)

# B19 series: Income tables
b19_tables <- all_vars_5 |>
  dplyr::filter(stringr::str_starts(name, "B19")) |>
  dplyr::distinct(concept) |>
  dplyr::arrange(concept)

base::print(base::paste("B20 (Earnings) tables:", base::nrow(b20_tables)))
base::print(base::paste("B08 (Commuting/Occupation) tables:", base::nrow(b08_tables)))
base::print(base::paste("B19 (Income) tables:", base::nrow(b19_tables)))
```

```{r display-key-tables}
# Display most promising table series
key_tables <- dplyr::bind_rows(
  b20_tables |> dplyr::mutate(series = "B20"),
  b08_tables |> dplyr::mutate(series = "B08"), 
  b19_tables |> dplyr::mutate(series = "B19")
) |>
  dplyr::select(series, concept)
...
examine_table <- function(table_pattern) {
  all_vars_5 |>
    dplyr::filter(stringr::str_detect(name, table_pattern)) |>
...
base::print("B20017 Variables (Median Earnings by Sex and Work Experience):")
```
...
#   type = "pums",
#   year = 2023,
#   variables = base::c("SEX", "MAR", "OCCP", "WAGP", "AGEP"),
#   survey = "acs5",
#   state = "CA"
#   )
...
all_vars_with_tables <- all_vars_5 |>
  dplyr::mutate(
    ...
    table_name = stringr::str_extract(name, "^[A-Z][0-9]{5}[A-Z]*"),
    # Extract the sequence number and estimate/MOE indicator
    sequence = stringr::str_extract(name, "_[0-9]{3}[EM]$"),
    # Clean sequence to just the number
    sequence_num = base::as.numeric(stringr::str_extract(sequence, "[0-9]{3}")),
    # Estimate or Margin of Error
    est_moe = stringr::str_extract(sequence, "[EM]")
  )
...
tables_summary <- all_vars_with_tables |>
  dplyr::filter(!base::is.na(table_name)) |>
  dplyr::group_by(table_name, concept) |>
  dplyr::summarise(
    n_variables = dplyr::n(),
    min_sequence = base::min(sequence_num, na.rm = TRUE),
    max_sequence = base::max(sequence_num, na.rm = TRUE),
    .groups = "drop"
  ) |>
  dplyr::arrange(table_name)

base::print(base::paste("Total number of tables:", base::nrow(tables_summary)))
...
get_table_variables <- function(table_name) {
  all_vars_with_tables |>
    dplyr::filter(stringr::str_detect(name, base::paste0("^", table_name))) |>
    dplyr::select(name, label, concept, sequence_num, est_moe) |>
    dplyr::arrange(sequence_num)
}

# Example: Get all variables in table B20017 (earnings by sex and experience)
b20017_all <- get_table_variables("B20017")
base::print(base::paste("Table B20017 has", base::nrow(b20017_all), "variables"))
```
...
find_tables_by_keyword <- function(keyword) {
  tables_summary |>
    dplyr::filter(stringr::str_detect(base::tolower(concept), base::tolower(keyword))) |>
    dplyr::arrange(dplyr::desc(n_variables))
}

# Find all tables related to earnings
earnings_tables <- find_tables_by_keyword("earning")
base::print(base::paste("Found", base::nrow(earnings_tables), "tables related to earnings"))
...
find_variable_table <- function(variable_name) {
  all_vars_with_tables |>
    dplyr::filter(name == variable_name) |>
    dplyr::select(name, table_name, label, concept)
}
...
base::print(base::paste("Data source:", api_url))
...
explore_table <- function(table_name) {
  ...
  estimates_only <- table_vars |>
    dplyr::filter(est_moe == "E")
  
  # Create summary tibble for display
  summary_info <- tibble::tibble(
    Attribute = base::c("Table", "Concept", "Total Variables", "Estimates", "Margins of Error"),
    Value = base::c(
      table_name,
