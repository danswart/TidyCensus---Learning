...
OCCUPATION_GAP_ANALYSIS <- c("MED-Physicians", "MED-Surgeons", "MED-Dentists", "MED-Pharmacists", "LGL-Lawyers, And Judges, Magistrates, And Other Judicial Workers", "EDU-Postsecondary Teachers", "MGR-General And Operations Managers", "EDU-Secondary School Teachers", "MED-Registered Nurses", "ENG-Engineer", "FIN-Accountants And Auditors
", "MGR-Sales Managers")
...
education_desc_a <- if (!is.null(GROUP_A_EDUCATION_CODE)) {
  ...
  print(paste("education_desc_a built with codes:", desc))
  desc
} else if (!is.null(GROUP_A_EDUCATION_LABEL)) {
  desc <- paste0(" (Edu: ", GROUP_A_EDUCATION_LABEL, ")")
  print(paste("education_desc_a built with label:", desc))
  desc
} else if (!is.null(MIN_EDUCATION_CODE)) {
  desc <- paste0(" (Edu >= ", MIN_EDUCATION_CODE, ")")
  print(paste("education_desc_a built with min edu:", desc))
  desc
} else {
  print("education_desc_a empty")
  ""
}
...
education_desc_b <- if (!is.null(GROUP_B_EDUCATION_CODE)) {
  ...
  print(paste("education_desc_b built with codes:", desc))
  desc
...
cat("Sample size (Group A):", nrow(group_a_data), "\n")
cat("Sample size (Group B):", nrow(group_b_data), "\n\n")
...
# Enhanced function to filter PUMS data with education parameters (with debug prints)
filter_pums_data <- function(data, sex_code, mar_code = NULL, 
  ...
  cat("After base filters:", nrow(result), "rows\n")
  ...
  if (!is.null(edu_code)) {
    if (length(edu_code) == 1) {
      ...
      cat("After single SCHL code filter:", nrow(result), "rows\n")
    } else {
      result <- result %>% filter(SCHL %in% edu_code)
      cat("After multiple SCHL codes filter:", nrow(result), "rows\n")
    }
  }
  ...
  if (!is.null(min_edu)) {
    ...
    cat("After min SCHL filter:", nrow(result), "rows\n")
  }
...
if (nrow(top_occupations) > 0) {
  ...
  cat("Insufficient data for top occupations analysis.\n")
}
```
...
*Template Version: 2.1 - Enhanced with Educational Attainment and Error Handling*
